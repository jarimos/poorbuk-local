...
class PeepSo
{
    ...
    public static function php() {
        ...
        if(-1 == version_compare($php_current, $php_required)) {
            ...
            $message = "<strong>PeepSo can't be activated!</strong> You are using <strong>PHP $php_current</strong> - PeepSo requires PHP $php_required.x, but <strong>we recommend PHP $php_recommended.x</strong>.";
        }
        ...
        if($allow_activation) {
            ...
            if (-1 == version_compare($php_current, $php_recommended)) {
                $message = "You are using <strong>PHP $php_current</strong> - <strong>we recommend PHP $php_recommended.x</strong>.";
            }
        }
...
//				$post = get_post($repost)
    ...
    public function check_ajax_query()
    {
        ...
        // Downloader.
        if ('peepsodl' === $parts[ count($parts) - 1 ]) {
            remove_all_filters('wp_loaded');

            $url = isset($_GET['url']) ? $_GET['url'] : '';
            if (strpos($url, site_url()) === 0) {
                $file = str_replace(site_url(), untrailingslashit(ABSPATH), $url);
                if (file_exists($file)) {
    ...
    public function init_callback()
    {
        ...
        if( PeepSo::can_schedule_posts() || class_exists('PeepSoGroupsPlugin') || class_exists('PeepSoFriends')) {
...
//		}
    ...
    public function enqueue_scripts()
    {
        ...
        wp_register_script('peepso', FALSE, array('peepso-core'), PeepSo::PLUGIN_VERSION, TRUE);
    ...
    public function enqueue_scripts_data()
    {
        $data = apply_filters('peepso_data', array(
            ...
            'allow_non_ssl_embed' => PeepSo::get_option('allow_non_ssl_embed', 0),
...
// URLs and paths
    ...
    /**
     * Return download url for specific file.
     * @param  string $url
     * @return string
     */
    public static function get_download_url($url) {
        return site_url( 'peepsodl/?url=' . $url );
    }
