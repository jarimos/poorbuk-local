...
//		$wp_query->is_page = FALSE;
	...
	private function register_user()
	{
		...
		if (wp_verify_nonce($sNonce, 'register-form')) {
			...
			if (PeepSo::get_option('registration_confirm_email_field', 1)) {
				$email_verify = $input->val('email_verify', '');
	...
	public function enqueue_scripts()
	{
        ...
        if (1 === PeepSo::get_option('site_registration_enableterms', 0)) {
            $data['terms'] = stripslashes(nl2br(PeepSoSecurity::strip_content(PeepSo::get_option('site_registration_terms', ''))));
        }

        if (1 === PeepSo::get_option('site_registration_enableprivacy', 0)) {
            $data['privacy'] = stripslashes(nl2br(PeepSoSecurity::strip_content(PeepSo::get_option('site_registration_privacy', ''))));
        }
	...
	public function activate_account()
	{
        ...
        $input = new PeepSoInput();
        $key = $input->val('community_activation_code', $input->val('peepso_activation_code', NULL)); // Fallback activation code - see #3142
	...
	public static function description() {
	    return sprintf(__('Displays registration form for your community. You can decide which fields to show on the registration (requires %s).','peepso-core'),
            '<a href="https://www.peepso.com/core/#edd_download_43454" target="_blank"><nobr>'.__('Extended Profiles','peepso-core').' <i class="fa fa-external-link"></i></nobr></a>');
    }
