...
class PeepSoReactionsModel
{
	 ...
	 * @return bool
	 */
	public function user_reaction_reset()
	{
		global $wpdb;
	...
	public function user_reaction_set( $react_id )
	{
		...
		if( 0 == $react_id ) {
			...
			if( TRUE === $do_notify ) {
				// send LIKE email
				PeepSoMailQueue::add_message($owner_id, $mailq_data, sprintf(__('Someone liked your %s', 'peepso-core'), $activity_type), 'like_post', 'like_post', PeepSoActivity::MODULE_ID);
	...
	public function html_reactions_details()
	{
		...
		<a title="<?php echo __('Remove reaction','peepso-core');?>" id="ps-reaction-details-close" class="ps-reaction-details-close" href="#"
		   onclick="reactions.action_html_reactions(this, <?php echo $this->act_id; ?>); return false;">
	...
	public function html_before_comments()
	{
		...
		?>
			<div id="act-reactions-<?php echo $this->act_id; ?>"
			 class="ps-reactions__dropdown cstream-reactions-options ps-js-reaction-options ps-js-act-reactions-options--<?php echo $this->act_id; ?>"
			 data-count="">
			<div class="ps-reactions__list">
				...
				foreach ($this->reactions as $react_id => $reaction) :
					...
					<span class="ps-reactions__list-item">
						<a title="<?php echo $this->reaction($react_id)->title;?>" class="<?php echo $class;?>" href="#"
