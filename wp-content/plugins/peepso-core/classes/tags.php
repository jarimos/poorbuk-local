...
class PeepSoTags
{
	...
	public function shortcode_tag($atts, $content = '')
	{
		...
		return ($before_fullname  . sprintf('<a class="ps-tag__link" href="%s" data-hover-card="%d">%s</a>', $user->get_profileurl(), $user->get_id(), $name) . $after_fullname);
	}
		...
		if ($match) {
			...
			foreach ($this->user_ids as $user_id) {
				...
				if($user_id != $post_act->act_owner_id) {
					...
					PeepSoMailQueue::add_message($user_id, $data, __('Someone tagged you in a post', 'peepso-core'), 'tagged', 'tag', self::MODULE_ID);
		...
		if ($match) {
			...
			foreach ($this->user_ids as $user_id) {
		        ...
		        if ((!in_array($user_id, $follower) && ($user_id != $post_act->act_owner_id)) || ($post_act->act_owner_id == $user_id && intval($post_act->act_comment_object_id) > 0)) {
					...
					// TODO: need to use an editable email message, not a constant string
					// SpyDroid: the constant string is an email subject and not an editable email message, the template for editable email is the 4th parameter 'tagged'
					PeepSoMailQueue::add_message($user_id, $data, __('Someone tagged you in a comment', 'peepso-core'), 'tagged_comment', 'tag_comment', self::MODULE_ID);
	...
	public function config_email_tags($emails)
	{
		$emails['email_tagged'] = array(
			'title' => __('User Tagged In Post', 'peepso-core'),
			'description' => __('This will be sent to a user when tagged in post.', 'peepso-core')
		);

		$emails['email_tagged_comment'] = array(
			'title' => __('User Tagged In Comment', 'peepso-core'),
			'description' => __('This will be sent to a user when tagged in comment.', 'peepso-core')
		);
	...
	{
		$alerts['tags'] = array(
				'title' => __('Tags', 'peepso-core'),
				'items' => array(
					array(
						'label' => __('Someone tagged me in a Post', 'peepso-core'),
						'setting' => 'tag',
						'loading' => TRUE,
					),
					array(
						'label' => __('Someone tagged me in a Comment', 'peepso-core'),
						'setting' => 'tag_comment',
						'loading' => TRUE,
					)
				),
