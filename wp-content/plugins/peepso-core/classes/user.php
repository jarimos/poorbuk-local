...
class PeepSoUser
{
	...
	public static function set_stream_filters($input) {
	    if($input instanceof PeepSoInput) {
            ...
            $stream_id = $input->val('stream_id', $default);
            $stream_id_list = apply_filters('peepso_stream_id_list', array());
            if(!isset($stream_id_list[$stream_id])) {
                $stream_id = $default;
            }
            ...
            // show my posts
            $default = 1;
            $show_my_posts = $input->val('stream_filter_show_my_posts', $default);
