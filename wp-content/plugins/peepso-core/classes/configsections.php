...
class PeepSoConfigSections extends PeepSoConfigSectionAbstract
{
	...
	private function registration()
	{
        ...
        $this->args('descript', __('Enabled: users don\'t need to confirm their e-mail. It is not recommended unless your registrations are coming in via another plugin (WooCommerce, EDD etc).', 'peepso-core'));
        $this->set_field(
            'registration_disable_email_verification',
            __('Disable e-mail verification', 'peepso-core'),
            'yesno_switch'
        );

        // Enable Account Verification
        $this->args('descript', __('Enabled: users register, confirm their e-mail (optional) and must be accepted by an Admin. Users are notified by email when they\'re approved.<br/>Disabled: users register, confirm their email address and can immediately participate in your community.', 'peepso-core'));
        $this->set_field(
