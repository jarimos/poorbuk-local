...
class PeepSoMemberSearch extends PeepSoAjaxCallback
{
    ...
    public function search(PeepSoAjaxResponse $resp)
    {
        ...
        // Followed only
        $peepso_args['following'] = $this->_input->val('peepso_following', -1);
        ...
        // Blocked only
        $peepso_args['blocked'] = (int) $this->_input->val('blocked', 0);
    ...
    public function show_member($member)
    {
        ...
        if (get_transient('peepso_cache_'.$member->get_id().'_online')) {
            $online = PeepSoTemplate::exec_template('profile', 'online', array('PeepSoUser'=>$member,'class'=>'ps-user__status--sm'), TRUE);
        }
        ...
        echo '<div class="ps-members-item-avatar"><div class="ps-avatar">
			...
			<div class="ps-members-item-body">
				<a href="' , $member->get_profileurl(), '" class="ps-members-item-title" title="', strip_tags($member->get_fullname()), '" alt="', strip_tags($member->get_fullname()), ' avatar">'
    ...
    {
        echo '<a class="ps-avatar ps-avatar--full" href="' . $member->get_profileurl() . '" title="' . strip_tags($member->get_fullname()) . '">
				<img alt="' . strip_tags($member->get_fullname()) . ' avatar"
				src="' . $member->get_avatar() . '" class="ps-name-tips"></a>';
    ...
    public function show_latest_member($member)
    {
        $online = '';
        if (get_transient('peepso_cache_'.$member->get_id().'_online')) {
            $online = PeepSoTemplate::exec_template('profile', 'online', array('PeepSoUser'=>$member,'class'=>'ps-user__status--member'), TRUE);
        }

        echo '<a class="ps-avatar ps-avatar--full" href="' . $member->get_profileurl() . '" title="' . strip_tags($member->get_fullname()) . '">
				<img alt="' . strip_tags($member->get_fullname()) . ' avatar"
				src="' . $member->get_avatar() . '" class="ps-name-tips"> ' . $online . '</a>';
    ...
    public static function member_options($user_id, $profile = FALSE)
    {
        ...
        if ($blk->is_user_blocking(get_current_user_id(), $user_id)) {
            ...
            $options['unblock'] = array(
                ...
                'click' => 'ps_member.unblock_user(' . $user_id . ', this); return false;',
    ...
    public static function member_buttons($user_id)
    {
        ...
        foreach ($buttons as $name => $data) {
            ...
            if (isset($data['loading']))
                $member_buttons .= ' <img style="display:none" src="' . PeepSo::get_asset('images/ajax-loader.gif') .'" alt=""></span>';
