...
class PeepSoPostbox extends PeepSoAjaxCallback
{
	...
	public function post_interactions($params = array())
	{
		...
		if (isset($inter['privacy'])) {
			...
			$privacy_option = '<a href="#" id="postbox-acc-%1$d" data-option-value="%1$d" onclick="return false;"><i class="ps-icon-%2$s"></i><span>%3$s</span></a>';

			$inter['privacy']['extra'] .= '<div class="ps-postbox-privacy ps-privacy-dropdown ps-dropdown__menu" style="display: none;">';

			foreach ($privacy_settings as $value => $setting) {
                $inter['privacy']['extra'] .= sprintf($privacy_option, $value, $setting['icon'], $setting['label']);
            }

			$inter['privacy']['extra'] .= '</div>';
		}
		...
		foreach ($inter as $key => $data) {
            ...
            if (!empty($data['click'])) {
                echo '<a';
                if (isset($data['title']))
                    echo ' class="pstd-secondary ps-tooltip ps-tooltip--postbox" data-tooltip="', esc_attr($data['title']), '"';
                else
                    echo ' class="pstd-secondary ps-tooltip ps-tooltip--postbox"';
	...
	public function postbox_type_interaction($interactions, $params = array())
	{
		...
		$html = '<div class="ps-dropdown__menu ps-js-postbox-type" style="display:none">';
		$icons = array();
		foreach ($types as $type => $data) {
			$icons[] = $data['icon'];
			$html .= '<a role="menuitem" class="ps-dropdown__group" data-option-value="' . $type . '">';
