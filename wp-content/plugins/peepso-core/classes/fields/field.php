...
class PeepSoField
{
	...
	protected $field_meta_keys = array(
		// rendering
		'is_core',					// INT 1,2 (optional)		1 - WP core, 2 - PeepSo
		'order',					// INT
		'class',					// string
		'method',					// string
		'method_form',				// string
	...
	{
	    // fallback in case someone passed an entire user object
		...
		foreach ($this->field_meta_keys as $meta) {
			...
			if('privacywarningtext' == $meta && !strlen($this->meta->$meta)) {
			    $this->meta->$meta = __('Stay safe! Be careful about disclosing personal information.','peepso-core');
            }
		}
	...
	public function validate()
	{
		if( count((array) $this->meta->validation) ) {
			...
			if(is_object($this->meta->validation) && count((array) $this->meta->validation)) {
	...
	public function get_acc()
	{
		...
		// Searchable fields default to Site Members
		if(1 == $this->prop('meta', 'searchable')) {
		    return PeepSo::ACCESS_MEMBERS;
        }
	...
	{
		$ret  = '<div class="ps-form__field-inner">';
	...
	public function render_access()
	{
        ...
        if( 1 == $this->meta->user_disable_acc) {
			?>
			<span class="ps-profile-privacy--disabled ps-tooltip" data-tooltip="<?php _e('Some fields privacy is defined by the administrators', 'peepso-core'); ?>">
		...
		<div class="ps-profile-privacy">
			<div class="ps-privacy-dropdown ps-js-dropdown">
				...
				<button id="acc-<?php echo $this->input_args['name'];?>" type="button"
						class="ps-btn ps-btn-small ps-dropdown__toggle ps-js-dropdown-toggle"
						aria-label="<?php _e('Edit privacy of ' . $this->title, 'peepso-core'); ?>">
				...
				<div class="ps-dropdown__menu ps-js-dropdown-menu">
					<?php foreach ($access_settings as $acc_key => $acc_value) { ?>
						<a href="#" id="<?php echo $this->id . '-acc-' . $acc_key ?>" data-option-value="<?php echo $acc_key; ?>" onclick="profile.change_privacy(this); return false;">
							<i class="ps-icon-<?php echo $acc_value['icon']; ?>"></i>
