...
class PeepSoFieldSelectMulti extends PeepSoFieldSelectSingle {
	...
	protected function _render($echo = false)
	{
		...
		if (!count($this->value) || ($this->is_registration_page)) {
	...
	protected function _render_form_checklist()
	{
		...
		foreach ($options as $k => $v) {
			...
			$option = '<div class="ps-checkbox"><input id="%4$s-%1$s" name="%4$s" type="checkbox" %3$s value="%1$s" ' . $this->_render_input_checklist_args() . ' /> <label for="%4$s-%1$s">%2$s</label></div>';
	...
	protected function _render_form_checklist_register()
	{
		...
		foreach ($options as $k => $v) {
			...
			$option = '<div class="ps-checkbox"><input name="%4$s[]" type="checkbox" %3$s value="%1$s" id="%1$s" ' . $this->_render_input_checklist_register_args() . ' /> <label for="%1$s">%2$s</label></div>';
