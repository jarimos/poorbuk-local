...
class PeepSoMoods {
	...
	public function filter_postbox_interactions($out_html = array())
	{
		...
		{
			$mood_list .= "
				<a class='mood-list' id='postbox-mood-{$id}' href='#' data-option-value='{$id}' data-option-display-value='{$mood}' onclick='return false;'>
		...
		$mood_data = array(
			...
			'extra' => "<div id='postbox-mood' class='ps-dropdown__menu ps-dropdown__menu--moods ps-moods__dropdown ps-js-postbox-mood' style='display:none'>
							<div class='ps-moods__dropdown-inner'>
								<div class='ps-moods__dropdown-list'>
									{$mood_list}
									<button id='postbox-mood-remove' class='ps-moods__remove' style='display:none' title='{$mood_remove}'><i class='ps-icon-remove'></i></button>
								</div>
							</div>
						</div>{$mood_ux}"
		);
	...
	public function filter_post_extras( $extras = array() )
	{
		...
		{
			ob_start();?>
			<span class="ps-post__mood">
				<i class="ps-emoticon <?php echo $this->class_prefix . $post_mood_id;?>"></i>
				<span><?php echo __(' feeling ', 'peepso-core') . ucwords($post_mood);?></span>
			</span>
			<?php
			$extras[] = ob_get_clean();
		}
