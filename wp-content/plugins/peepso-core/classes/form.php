...
class PeepSoForm
{
	...
	public function render_fields($fields)
	{
		foreach ($fields as $name => $field) {
			...
			if ('hidden' === $field['type']) {
				...
				echo '<ul class="ps-form__error"', ($field['valid'] ? ' style="display:none"' : ''), '>';
				if (!$field['valid']) {
	...
	public function render_field($field)
	{
		...
		elseif ('title' === $field['type'])
			...
			if (!$field['valid']) {
				$sField .= '<ul class="ps-form__error">';
	...
	private function _render_field_type($name, $field)
	{
		...
		switch ($field['type'])
		{
			...
			case 'access-profile-post':
				...
				$sField .=	'<div class="ps-dropdown ps-js-dropdown ps-js-dropdown--privacy">';
				$sField .=		'<input type="hidden" name="' . $name . '" value="' . $field['value'] . '" />';
				$sField .=		'<button id="acc-' . $name . '" type="button" class="ps-btn form-control ps-dropdown__toggle ps-js-dropdown-toggle">';
				...
				$sField .=		'</button>';
				$sField .=		'<div class="ps-dropdown-menu ps-dropdown__menu ps-js-dropdown-menu" style="display:none">';
