	...
	initActivityFilter() {
		...
		this.$input
			.on( 'input', e => {
				let el = e.target,
					keyword = el.value,
					filtered = keyword.replace( HASHTAG_EVERYTHING ? /[#\s<]/g : /[^a-z0-9]/gi, '' ),
		...
		let url = decodeURIComponent( window.location.href ),
			pattern = HASHTAG_EVERYTHING ? /\/\??hashtag\/([^#\s<\/]+)/ : /\/\??hashtag\/([a-z0-9]+)/i,
