(function( $ ) {
    ...
    $container.on('input', '.ps-reaction-notification', function() {
        $( this ).closest('.ps-js-reactionconf').find('.ps-reaction-hint-inner').html($(this).val());
    });

    // input handler
    $container.on('input', '.ps-js-reaction input[type=text]', function() {
        ...
        .find('.ps-js-reaction input[type=text]').each(function() {
    ...
    $container.sortable({
        ...
        update: _.throttle(function() {
            ...
            $PeepSo.postJson('adminConfigreactions.reorder', { id: JSON.stringify(id) }, function( json ) {});
        }, 3000 )
    });
    ...
    var toggleAllCallback = _.debounce(function( status ) {
        ...
        if ( status === 0 ) {
            $reactions.find('.ps-js-reaction-toggle').removeClass('fa-compress').addClass('fa-expand');
        } else {
            $reactions.find('.ps-js-reaction-toggle').removeClass('fa-expand').addClass('fa-compress');
        }
    ...
    function updateToggleAllButton( status ) {
        ...
        if ( typeof status === 'undefined' ) {
            ...
            visible = 0;
            status = 0;
            $('.ps-js-reactions-container').find('.ps-js-reaction').each(function() {
                len++;
                if ( $(this).is(':visible') ) {
                    visible++;
                }
    ...
    var updatereactionVisibility = _.debounce(function( id, status ) {
        ...
        if ( id !== 'all' ) {
            ...
            $('.ps-js-reactions-container').children('.postbox').each(function() {
                ids.push( $(this).data('id') )
            });
        }

        updatereactionXHR[ id ] && updatereactionXHR[ id ].ret && updatereactionXHR[ id ].ret.abort();
        updatereactionXHR[ id ] = $PeepSo.postJson('adminConfigreactions.update', { prop: 'box_status', id: JSON.stringify( ids ), status: status }, function( json ) {
            // Do nothing
        });
    }, 500 );
    ...
    $('.ps-js-reaction-new').on('click', function(e) {
        ...
        peepso.postJson('adminConfigreactions.create', { type: $(this).data('value') }, function( response ) {
            if ( response.success ) {
                $button.html($oldHtml);
