...
//	});
	...
	if ( $pref.length ) {
		...
		$pref.find('.ps-preferences__checkbox').find('input[type=checkbox]').off('click.savepref').on('click.savepref', profile.save_notification );
	}
	...
	if ( $items.length ) {
		$items.each(function() {
			...
			if ( $item.find( '.peepso-markdown' ).length ) {
				var html = ps_observer.apply_filters( 'peepso_parse_content', $item.html() );
				$item.html( html );
			}
		});
	}
		...
		add: function(e, data) {
			...
			if (response.success) {
				...
				jQuery(".ps-focus-image-mobile")
					.attr('style', 'background:url(' + imageUrl + ') no-repeat center center;');

				// Announce update cover image.
				ps_observer.do_action('profile_cover_update', peepsodata.userid, imageUrl );
...
PsProfile.prototype.edit_field = function( elem )
{
	...
	if ( type === 'checkbox' ) {
		...
		if ( $counter.length ) {
			$counter.show();
			$input.off('input').on('input', _.throttle(function( e ) {
				$counter.html( e.target.value.length );
			}, 500)).triggerHandler('input');
		}
		$input.focus();
	} else {
		$ct.data('original-value', $input.val() );
	...
	$PeepSo.postJson('profilefieldsajax.savefield', req, function(json) {
		...
		if ( json.success ) {
			...
			if ( data.display_value != undefined ) {
				var displayValue = ps_observer.apply_filters( 'peepso_parse_content', data.display_value );
			...
			ps_observer.do_action( 'profile_field_updated' );
...
PsProfile.prototype.change_privacy = function( elem ) {
	...
	// Map icons.
	$elem.parent().children( '[data-option-value]' ).each(function() {
		var $a = jQuery( this ),
	...
	// Post update.
	peepso.postJson( 'profilefieldsajax.save_acc', {
		user_id: peepsodata.currentuserid,
		view_user_id: peepsodata.userid,
		id: id,
		acc: newVal
...
PsProfile.prototype.profile_deletion = function() {
	...
	if ( ! $popup.length ) {
		...
		$popup.on( 'click', '.ps-js-submit', jQuery.proxy(function( e ) {
			var $loading = jQuery( e.currentTarget ).find( 'img' ).show(),
				password = $popup.find( 'input[type=password]' ).val(),
				params = { password: password };

			this.deleting_profile = true;
			peepso.postJson( 'profile.delete_profile', params, jQuery.proxy(function( json ) {
				...
				if ( json.success ) {
					...
					$popup.find( '.ps-js-error' ).html( json.errors[0] ).show();
				}
			}, this ) );
		}, this ) );
	}
...
PsProfile.prototype.request_account_data = function() {
	...
	if ( ! $popup.length ) {
		...
		$popup.on( 'click', '.ps-js-submit', jQuery.proxy(function( e ) {
			...
			peepso.postJson( 'profile.request_account_data', params, jQuery.proxy(function( json ) {
...
PsProfile.prototype.download_account_data = function() {
	...
	if ( ! $popup.length ) {
		...
		$popup.on( 'click', '.ps-js-submit', jQuery.proxy(function( e ) {
			...
			this.requesting_data = true;
			peepso.postJson( 'profile.download_account_data', params, jQuery.proxy(function( json ) {
				this.requesting_data = false;
...
PsProfile.prototype.delete_account_data_archive = function() {
	...
	if ( ! $popup.length ) {
		...
		$popup.on( 'click', '.ps-js-submit', jQuery.proxy(function( e ) {
			...
			peepso.postJson( 'profile.delete_account_data_archive', params, jQuery.proxy(function( json ) {
