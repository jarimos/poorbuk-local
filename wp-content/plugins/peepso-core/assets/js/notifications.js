( function( $, factory ) {
	...
	function PsNotification() {
		...
		peepso.observer.add_action( 'notification_start', start );
	...
	PsNotification.prototype = {
				...
				.postJson(
					...
					$.proxy( function( json ) {
						...
						if ( json.success && ! json.session_timeout ) {
							...
							peepso.observer.do_action( 'notification_update', json );
						}
					}, this )
				);
		},
		...
		hide: function( note_id ) {
			...
			$elems = $( '.ps-js-notifications' )
				...
				.map( function() {
					return $( this )
						.parent( '.ps-notification__wrapper' )
						.get( 0 );
				} );
			...
			peepso.postJson(
				...
				$.proxy( function( json ) {
					...
					if ( json.success ) {
						$elems.remove();
						peepso.observer.do_action( 'notification_restart' );
					} else {
						$elems.css( 'opacity', '' );
					}
				}, this )
		...
		start: function() {
			...
			peepso.observer.add_filter( 'pschat_mark_as_read', this.restart, 10, 1, this );
			peepso.observer.add_action( 'notification_restart', this.restart, 10, 1, this );
		},
...
( function( $ ) {
	function PsPopoverNotification( elem, options ) {
		...
		this.init = function( opts ) {
			...
			this.opts = peepso.observer.apply_filters( 'peepso_notification_plugin_options', _opts );
			...
			if ( footerLinks ) {
				...
				footerBtnWidth = [ false, false, '50%', '33%', '25%' ][ footerLinks.length ];
				...
				footerLinks = _.map( footerLinks, function( value, index ) {
					return [
						...
						value,
						'"',
						footerBtnWidth ? ' style="float:left; width:' + footerBtnWidth + '"' : '',
						'>',
						footerTexts[ index ],
						'</a>'
					].join( '' );
				...
				this.popover_footer = $(
					'<div class="ps-popover-footer app-box-footer ps-clearfix"></div>'
				);
				this.popover_footer.append( footerLinks.join( '' ) );
				this.popover_footer.appendTo( this.popover );
			}
			...
			this.popover_footer.on(
				...
				$.proxy( function( e ) {
					...
					$btn.html(
						this._unreadOnly ? peepsodata.show_all_text : peepsodata.show_unread_only_text
					);
		...
		this.load_page = function( callback ) {
			if ( false === this._content_is_fetched ) {
				...
				if ( ! loading.length ) {
					loading = $(
						"<div class='ps-popover-loading'><img src='" + peepsodata.loading_gif + "'/></div>"
					);
	...
	$.fn.psnotification = function( methodOrOptions ) {
		return this.each( function() {
			if ( ! $.data( this, 'plugin_psnotification' ) ) {
				$.data( this, 'plugin_psnotification', new PsPopoverNotification( this, methodOrOptions ) );
			} else {
