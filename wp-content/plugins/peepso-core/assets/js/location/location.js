...
( function( root, $, factory ) {
	...
	PsLocation.prototype.init = function() {
		...
		this.$container = $( '#pslocation', this.$postbox ).on( 'click', function( e ) { e.stopPropagation() } );
	...
	*/
	...
	PsLocation.prototype.on_select_location = function() {
		...
		this.$container.addClass( 'hidden' );
		this.$container.closest( '.ps-postbox__menu-item' ).addClass( 'active' );
	...
	PsLocation.prototype.on_remove_location = function() {
		...
		this.$postboxcontainer.find( 'span#postlocation' ).remove();
		this.$container.addClass( 'hidden' );
		this.$container.closest( '.ps-postbox__menu-item' ).removeClass( 'active' );
	...
	PsLocation.prototype.toggle_input = function() {
		if ( this.$container.hasClass( 'hidden' ) ) {
			...
			jQuery( document ).off( 'click.ps-postbox-location' );
			setTimeout( jQuery.proxy(function() {
				var evtName = 'click.ps-postbox-location';
				jQuery( document )
					.off( evtName )
					.one( evtName, jQuery.proxy(function() {
						this.$container.addClass( 'hidden' );
						jQuery( document ).off( 'click.ps-postbox-location' );
					}, this ) );
			}, this ), 1 );
	...
	PsLocation.prototype.show_map = function( lat, lng, name ) {
		peepso.lightbox(
			[
				{
					content:
						'<div class="ps-js-mapct" style="width:700px;height:400px;display:inline-block" />'
				}
			],
			{
				simple: true,
	...
	PsLocation.prototype.detect_location = function() {
		...
		return $.Deferred( function( defer ) {
			if ( window.location.protocol !== 'https:' ) {
				...
				that
					.detect_location_by_device()
					.done( function( lat, lng ) {
						defer.resolve( lat, lng );
					} )
					.fail( function() {
						that
							.detect_location_by_gmap_api()
							.done( function( lat, lng ) {
								defer.resolve( lat, lng );
							} )
							.fail( function() {
		...
		return $.Deferred(
			$.proxy( function( defer ) {
				...
				if ( this._client_location ) {
					...
					$.post( 'https://www.googleapis.com/geolocation/v1/geolocate?key=' + api_key, function(
						coords
					) {
