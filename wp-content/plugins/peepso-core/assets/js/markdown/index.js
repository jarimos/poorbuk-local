...
const compileMarkdown = html => {
	...
	// Workaround on nested markdown wrapper due to a rendering bug.
	$markdown = $markdown.map( function() {
		let nested = $( this ).children( '.peepso-markdown' ).length;
		return nested ? null : this;
	} );
	...
	$markdown.each( function() {
		...
		try {
			...
			html = html.replace( /\[([^\[\]]+)\]\((<a[^\(\)]+<\/a>)\)/gi, function( match, title, link ) {
