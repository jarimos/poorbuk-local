(function( $ ) {
    ...
    // make sure max year range is greater than min year range
    $container.on('change', 'select[data-prop-name=date_range_min]', function( e ) {
        var $elem = $(this),
            $pair = $elem.closest('.ps-tab__content').find('select[data-prop-name=date_range_max]');

        if (+$pair.val() < +$elem.val()) {
            $pair.val( $elem.val() ).trigger('change');
        }
    });

    // make sure min year range is smaller than max year range
    $container.on('change', 'select[data-prop-name=date_range_max]', function( e ) {
        var $elem = $(this),
            $pair = $elem.closest('.ps-tab__content').find('select[data-prop-name=date_range_min]');
