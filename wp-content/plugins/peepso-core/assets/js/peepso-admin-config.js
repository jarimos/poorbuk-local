jQuery(function( $ ) {
	...
	// Toggle ultimate bundle license field.
	$( 'input[name=bundle_license]' ).on( 'input focus blur', _.throttle(function( e ) {
		var $statuses = $('.license_status_check'),
			bundleLicense = this.value,
			trimmedLicense = $.trim( bundleLicense ),
	...
	$( 'input[name=brute_force_enable]' ).on( 'click', _.throttle(function() {
		var $first = $( 'select[name=brute_force_max_retries]' ),
			$last = $( 'select[name=brute_force_reset_retries]' ),
			$wrapper;
	...
	$(document).ready(function() {
		...
		$( 'input[name=reporting_notify_email]' ).on( 'click', _.throttle(function() {
			var $list = $( 'textarea[name=reporting_notify_email_list]' ),
				$wrapper = $list.closest( '.form-group' );
		...
		// Toggle reporting
		$( 'input[name=site_reporting_enable]' ).on( 'click', _.throttle(function() {
			var $reason = $( 'textarea[name=site_reporting_types]' ),
				$email = $( 'input[name=reporting_notify_email]' ),
				$list = $( 'textarea[name=reporting_notify_email_list]' ),
				$wrapper = $reason.add( $email ).add( $list ).closest( '.form-group' );

			if ( this.checked ) {
				$wrapper.show();
				$email.triggerHandler( 'click' );
			} else {
				$wrapper.hide();
			}
		}, 100 ) ).triggerHandler( 'click' );
			...
			$avatars_wp_only.on("change", function() {
				if ($(this).is(":checked")) {
					$('#field_avatars_peepso_only').fadeOut(hide_animation_speed)
						.find('input[name=avatars_peepso_only]').attr('disabled', 'disabled');
					...
					$('#field_avatars_peepso_only').fadeIn(hide_animation_speed)
						.find('input[name=avatars_peepso_only]').removeAttr('disabled');
