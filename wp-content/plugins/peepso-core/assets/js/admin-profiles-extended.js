(function( $ ) {
	...
	$btn.off( 'click' ).on( 'click', function( e ) {
		...
		if ( $dropdown.is( ':visible' ) ) {
			...
			$dropdown.show();
			$doc.one( evtName, function() { $dropdown.hide() });
		}
	});
	...
	$container.on('click', '.ps-js-field-duplicate', function( e ) {
		...
		if ( confirm('Are you sure want to duplicate this field?') ) {
			var id = $(this).data('id');
			peepso.postJson('adminextendedprofiles.duplicate_field', { id: id }, function( response ) {
				if ( response.success ) {
	...
	$container.on('click', '.ps-js-reset-privacy', function( e ) {
		...
		$btn.data('progress', 1 ).css({ opacity: .5 });
		...
		$PeepSo.postJson('adminExtendedProfiles.reset_privacy', { id: $sel.data('parent-id') }, function( json ) {
	...
	function saveOptionState( id ) {
		var $ct = $container.find('.ps-js-field').filter('[data-id="' + id + '"]'),
			$fields = $ct.find('.ps-js-options').children('.ps-js-fieldconf').find('input[type=text]'),
