...
<div class="peepso">
	...
	<div class="ps-register ps-register--activate">
		<div class="ps-register__header">
			<h3 class="ps-register__header-title"><?php _e('Account Activation', 'peepso-core'); ?></h3>
			<p><?php _e('Please enter your activation code below to enable your account.', 'peepso-core'); ?></p>
		</div>
			...
			<form class="ps-form ps-form--register-activate" name="resend-activation" action="<?php PeepSo::get_page('register'); ?>?activate" method="post">
				<div class="ps-form__container">
					<div class="ps-form__row">
						<label for="activation" class="ps-form__label"><?php _e('Activation Code:', 'peepso-core'); ?>
							<span class="required-sign">&nbsp;*<span></span></span>
						</label>
						<div class="ps-form__field">
							<?php
								$input = new PeepSoInput();
								$value = $input->val('community_activation_code', $input->val('peepso_activation_code', '')); // Fallback activation code - see #3142
							?>
							<input type="text" name="community_activation_code" class="ps-input" value="<?php echo $value; ?>" placeholder="<?php _e('Activation code', 'peepso-core'); ?>" />
						</div>
					</div>
