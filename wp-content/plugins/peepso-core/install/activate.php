...
{fromfirstname} tagged you in a comment!
		...
		$aRet = array(
			'activities' => "
				CREATE TABLE `activities` (
					...
					INDEX `comment_object_id` (`act_comment_object_id`),
					INDEX `comment_module_id` (`act_comment_module_id`)
				) ENGINE=InnoDB",
			'activity_hide' => "
			...
			'notifications' => "
				CREATE TABLE `notifications` (
					...
					INDEX `external` (`not_external_id`),
					INDEX `read` (`not_read`)
				) ENGINE=InnoDB",
			'report' => "
				CREATE TABLE `report` (
			...
			'users' => "
				CREATE TABLE `users` (
					...
					INDEX `role` (`usr_role`),
					INDEX `avatar_custom` (`usr_avatar_custom`)
				) ENGINE=InnoDB",
			'ranking' => "
			...
			'brute_force_attempts_logs' => "
				CREATE TABLE `login_failed_attempts_logs` (
					...
					`attempts_ip` 			VARCHAR(255) NOT NULL DEFAULT '',
