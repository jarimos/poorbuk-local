...
//			PeepSoMailQueue::add($owner_id, $data, __('Someone Commented on Your Post', 'peepso-core'), 'usercomment');
    ...
    public function savecomment(PeepSoAjaxResponse $resp)
    {
        ...
        $post_content = $this->_input->val('post');
...
//			'posts_per_page' => 1,
    ...
    {
        if (strpos($query, PeepSoActivityStream::CPT_POST) === FALSE && strpos($query, PeepSoActivityStream::CPT_COMMENT) === FALSE) {
            return $query;
        }
        global $wpdb;
        ...
        $where = [];
        $where[] = "$wpdb->posts.post_type = '" . $post_type . "' AND ( $wpdb->posts.post_status = 'publish' OR $wpdb->posts.post_status = 'private' )";
        ...
        // regular search
        $search = $this->_input->val('search', NULL); // optional, string to search
        $search_mode = $this->_input->val('search_mode', 'exact'); // optional, whether to use exact phrase or any word
        ...
        // hashtag search
        $hashtag = $this->_input->val('search_hashtag', '');
...
//				PeepSoMailQueue::add($owner_id, $data, __('Someone liked your post', 'peepso-core'), 'likepost');
    ...
    public function savepost(PeepSoAjaxResponse $resp)
    {
        ...
        $user_id = $this->_input->int('uid');
        $post_content = $this->_input->val('post');
        $future = $this->_input->val('future', NULL);
    ...
    public function report(PeepSoAjaxResponse $resp)
    {
        ...
        if (PeepSo::check_permissions($this->get_post_owner($act_id), PeepSo::PERM_REPORT, $user_id)) {
            ...
            $reason_desc = $this->_input->val('reason_desc');
...
//        }
    ...
    public function post_age()
    {
        ...
        if($post_date > $curr_date) {
            ...
            if (0 == $config_absolute_dates || (24 == $config_absolute_dates && 24 * 3600 > ($curr_date - $post_timestamp_user_offset))) {
                ...
                echo date($config_date_format, $post_timestamp_user_offset);
            }
    ...
    public function show_posts_per_page(PeepSoAjaxResponse $resp)
    {
        ...
        $context = $this->_input->val('context');
        ...
        $this->stream_id = $this->_input->val('stream_id', PeepSo::get_option('stream_id_default'));
    ...
    public function post_action_title()
    {
        ...
        if ($post->post_author === $post->act_owner_id) {
            ...
            $title = sprintf('<a class="ps-stream-user" href="%s" data-hover-card="%d">%s</a> <span class="ps-post__subtitle ps-stream-action-title">%s</span> ',
                $user->get_profileurl(), $user->get_id(), $before_fullname . $user->get_fullname() . $after_fullname, $action);
        } else {
            $author = PeepSoUser::get_instance($post->post_author);
	...
	public static function is_permalink_ajax($return_post_id = FALSE) {
        ...
        $post_id = $PeepSoInput->val('post_id', 0);
